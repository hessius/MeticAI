...
def _derive_structural_tags(profile_obj: object) -> list[str]:
    ...
    if "Bloom" not in tags:
        for stage in stages:
            ...
            has_time_exit = any(
                (getattr(e, "type", "") or "").lower() == "time" for e in exits
            )
            if stype == "flow" and has_time_exit and pts:
                try:
                    y_vals = [
                        ...
                        for p in pts
                        if len(p) >= 2
                        and not (isinstance(p[1], str) and p[1].startswith("$"))
                    ]
    ...
    if "Pre-infusion" not in tags and len(stages) >= 2:
        ...
        if ftype == "power":
            ...
            try:
                ...
                if (ftype == "pressure" and max_y <= 4) or (
                    ftype == "flow" and max_y <= 2
                ):
                    tags.add("Pre-infusion")
            except (TypeError, ValueError):
                pass
...
def _parse_data_image_uri(image_uri: str) -> tuple[str, bytes]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid profile image data"
        ) from exc
    ...
    mime_type = (
        header[5:-7].strip().lower()
    )  # strip "data:" prefix and ";base64" suffix
...
def _is_allowed_machine_image_url(image_url: str) -> bool:
    ...
    return bool(
        ...
        and (allowed_resolved & candidate_resolved)
    )
...
async def upload_profile_image(
    profile_name: str, request: Request, file: UploadFile = File(...)
):
...
async def generate_profile_image(
    ...
    try:
        ...
        prompt_result = build_image_prompt_with_metadata(
            profile_name=profile_name, style=style, tags=tag_list
        )

        if not prompt_result or not isinstance(prompt_result, dict):
            logger.error(
                "Failed to build image prompt - prompt_result is invalid",
                extra={"request_id": request_id, "prompt_result": prompt_result},
            )
            raise HTTPException(
                status_code=500, detail="Failed to build image generation prompt"
            )
...
async def apply_profile_image(
    profile_name: str, request: Request, body: ApplyImageRequest
):
    ...
    try:
        ...
        if not image_data_uri.startswith("data:image/"):
            raise HTTPException(
                status_code=400, detail="Invalid image data - must be a data URI"
            )
        ...
        try:
            ...
            try:
                ...
                if img.format != "PNG":
                    raise HTTPException(
                        status_code=400, detail=f"Expected PNG format, got {img.format}"
                    )
            except HTTPException:
                raise
            except Exception as img_err:
                raise HTTPException(
                    status_code=400, detail=f"Invalid image data: {str(img_err)}"
                )
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to decode image data: {str(e)}"
            )
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "error": str(e),
                "message": "Failed to apply profile image",
            },
        )
...
async def proxy_profile_image(
    profile_name: str, request: Request, force_refresh: bool = False
):
    ...
    try:
            ...
            if partial_profile.name == profile_name:
                ...
                if image_path.startswith(("http://", "https://")):
                    ...
                    if not _is_allowed_machine_image_url(image_url):
                        raise HTTPException(
                            status_code=400,
                            detail="Profile image URL host is not allowed",
                        )
                else:
                    ...
                    if not meticulous_ip:
                        raise HTTPException(
                            status_code=500, detail="METICULOUS_IP not configured"
                        )
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    media_type = (
                        raw_content_type.split(";", 1)[0].strip() or "image/png"
                    )
        ...
        raise HTTPException(
            status_code=502, detail="Failed to fetch image from machine"
        )
    except Exception as e:
        logger.error(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch profile image: {str(e)}"
        )


@router.get("/api/profile/{profile_name:path}/target-curves")
async def get_profile_target_curves(profile_name: str, request: Request):
...
async def get_profile_info(
    profile_name: str, request: Request, include_stages: bool = False
):
            ...
            if partial_profile.name == profile_name:
                ...
                if include_stages:
                    ...
                    if hasattr(full_profile, "variables") and full_profile.variables:
                        result["profile"]["variables"] = _serialise(
    ...
    try:
        ...
        if new_name is not None:
            if not isinstance(new_name, str) or not new_name.strip():
                raise HTTPException(
                    status_code=400, detail="Profile name must be a non-empty string"
                )
        ...
        if temperature is not None:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Temperature must be a number"
                )
            if temperature > 100:
                raise HTTPException(
                    status_code=400, detail="Temperature must not exceed 100 °C"
                )
        ...
        if final_weight is not None:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Final weight must be a number"
                )
            if final_weight <= 0:
                raise HTTPException(
                    status_code=400, detail="Final weight must be greater than 0"
                )
        ...
        if all(
            v is None for v in [new_name, temperature, final_weight, variables, author]
        ):
            raise HTTPException(
                status_code=400, detail="At least one field to update is required"
            )
        ...
        if hasattr(full_profile, "error") and full_profile.error:
            raise HTTPException(
                status_code=502, detail=f"Failed to fetch profile: {full_profile.error}"
            )
        ...
        if (
            ...
            and hasattr(full_profile, "variables")
            and full_profile.variables
        ):
            incoming = {
                v["key"]: v["value"] for v in variables if "key" in v and "value" in v
            }
...
async def list_machine_profiles(request: Request):
    ...
    try:
        logger.info(
            "Fetching all profiles from machine", extra={"request_id": request_id}
        )
            ...
            try:
                ...
                try:
                    ...
                    entries = (
                        ...
                        else history.get("entries", [])
                    )
                    profile_name = getattr(full_profile, "name", None) or getattr(
                        partial_profile, "name", None
                    )
                    in_history = any(
                        entry.get("profile_name") == profile_name for entry in entries
                    )
                except Exception:
                    pass
                ...
                profile_dict = {
                    ...
                    "author": getattr(
                        full_profile, "author", getattr(partial_profile, "author", None)
                    ),
                    "temperature": getattr(
                        full_profile,
                        "temperature",
                        getattr(partial_profile, "temperature", None),
                    ),
                    "final_weight": getattr(
                        full_profile,
                        "final_weight",
                        getattr(partial_profile, "final_weight", None),
                    ),
                    ...
                    try:
                        for entry in entries:
                            if entry.get("profile_name") == profile_dict["name"]:
                                profile_dict["user_preferences"] = entry.get(
                                    "user_preferences"
                                )
                                if entry.get("reply"):
        ...
        try:
            history = load_history()
            entries = (
                history if isinstance(history, list) else history.get("entries", [])
            )
            profiles = []
            seen: set[str] = set()
            for entry in entries:
...
async def get_machine_profile(profile_id: str, request: Request):
    ...
    try:
        ...
        if hasattr(profile, "error") and profile.error:
            raise HTTPException(
                status_code=502, detail=f"Machine API error: {profile.error}"
            )
        ...
        if (
            profile_json.get("final_weight") is not None
            and "final_weight" not in existing_keys
        ):
            variables.append(
                {
                    "key": "final_weight",
        ...
        if (
            profile_json.get("temperature") is not None
            and "temperature" not in existing_keys
        ):
            variables.append(
                {
                    "key": "temperature",
...
async def get_machine_profile_json(profile_id: str, request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Machine API error: {str(e)}")
...
async def import_profile(request: Request):
    ...
    try:
        ...
        if source == "machine":
            ...
            machine_id = (
                profile_json.get("id") if isinstance(profile_json, dict) else None
            )
            if machine_id:
                try:
        ...
        deep_convert_to_dict(profile_json)
        new_entry = {
            "id": entry_id,
        ...
        if source == "file":
            try:
                ...
                normalised_from_create = (
                    result.get("_normalised_json") if isinstance(result, dict) else None
                )
                machine_profile_id = (
                ...
                fetch_id = machine_profile_id or (normalised_from_create or {}).get(
                    "id"
                )
                if fetch_id:
                    try:
...
async def import_from_url(request: Request):
    ...
    try:
        ...
        logger.info(
            "Importing profile from URL: %s", url, extra={"request_id": request_id}
        )
            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                async with client.stream("GET", url) as resp:
                    ...
                    async for chunk in resp.aiter_bytes(8192):
                        ...
                        if total > max_size:
                            raise HTTPException(
                                status_code=413, detail="Response too large (max 5 MB)"
                            )
                        chunks.append(chunk)
                    content = b"".join(chunks)
        except httpx.TimeoutException:
            raise HTTPException(
                status_code=408, detail="Request timed out fetching URL"
            )
        ...
        if generate_description:
            try:
                ...
                logger.warning(
                    "Failed to generate description for URL import: %s",
                    e,
                    extra={"request_id": request_id},
                )
                from services.analysis_service import _build_static_profile_description
        ...
        new_entry = {
            ...
            "created_at": created_at,
            "profile_name": profile_name,
            "user_preferences": f"Imported from URL: {url}",
            "reply": reply,
        ...
        try:
            ...
            logger.info(
                "URL-imported profile uploaded to machine: %s",
                profile_name,
                extra={
                    "request_id": request_id,
                    "machine_profile_id": machine_profile_id,
            ...
            logger.warning(
                "Profile saved to history but failed to upload to machine: %s",
                exc,
                extra={"request_id": request_id, "error_type": type(exc).__name__},
            )
        logger.info(
            "Profile imported from URL successfully: %s",
            profile_name,
            extra={"request_id": request_id, "entry_id": entry_id, "source_url": url},
        )
        return {
            "status": "success",
            "entry_id": entry_id,
        ...
        logger.error(
            "Failed to import profile from URL: %s",
            str(e),
            exc_info=True,
            extra={"request_id": request_id, "error_type": type(e).__name__},
        )
...
async def import_all_profiles(request: Request):
        ...
        try:
            ...
            for idx, profile in enumerate(profiles_to_import, 1):
                ...
                try:
                    ...
                    if generate_description:
                        try:
                            reply = await _generate_profile_description(
                                profile_json, request_id
                            )
                        except Exception as e:
                            logger.warning(
                                f"Failed to generate description for {profile_name}: {e}"
                            )
                            from services.analysis_service import (
                                _build_static_profile_description,
                            )
                    ...
                    new_entry = {
                        ...
                        "content_hash": compute_content_hash(profile_json)
    ...
    return StreamingResponse(
        generate_import_stream(), media_type="application/x-ndjson"
    )
...
async def list_orphaned_history_entries(request: Request):
    ...
    try:
        ...
        machine_names: set[str] = {getattr(p, "name", "") for p in profiles_result}
        ...
        for entry in entries:
            ...
            if (
                profile_name
                and profile_name not in machine_names
                and not is_temp_profile(profile_name)
            ):
                orphaned.append(
                    {
                        "id": entry.get("id"),
...
async def accept_sync_update(
    profile_id: str, request: Request, ai_description: bool = False
):
    ...
    try:
        ...
        if not target_entry:
            raise HTTPException(
                status_code=404, detail="No history entry found for this profile"
            )
        ...
        if ai_description:
            try:
                new_reply = await _generate_profile_description(
                    profile_dict, request_id
                )
            except Exception as e:
                logger.warning(
...
async def sync_status(request: Request):
    ...
    try:
        ...
        orphan_count = sum(
            1
            for entry in entries
            if entry.get("profile_name", "")
            and entry.get("profile_name", "") not in machine_names
        )
                ...
                try:
                    ...
                    try:
                        ...
                        error_details.append(
                            f"{entry_id}: machine fetch failed ({exc}), normalize failed ({inner_exc})"
                        )
            else:
...
Remember: NO information should be lost in this conversion!"""
        ...
        return {"status": "success", "converted_description": converted_description}
...
async def regenerate_profile_description(entry_id: str, request: Request):
    ...
    try:
        ...
        if not profile_json:
            raise HTTPException(
                status_code=400, detail="No profile JSON data available for this entry"
            )
...
async def _schedule_next_recurring(schedule_id: str, schedule: dict):
    ...
    if not next_time:
        logger.warning(
            f"Could not calculate next occurrence for recurring schedule {schedule_id}"
        )
        return
    ...
    for schedule_id, shot in persisted_shots.items():
        try:
            ...
            if scheduled_time <= now:
                logger.info(
                    f"Skipping expired shot {schedule_id} (scheduled for {scheduled_time_str})"
                )
                continue
            ...
            async def execute_scheduled_shot(
                ...
                ph=preheat,
                delay=shot_delay,
                was_preheating=(current_status == "preheating"),
            ):
                try:
                    ...
                    if was_preheating:
                        logger.info(
                            f"Restored shot {sid} was already preheating, waiting {delay:.0f}s until shot time"
                        )
                        ...
                        if preheat_delay > 0:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Preheat failed for scheduled shot {sid}: {e}"
                                )
            ...
            logger.error(
                f"Failed to restore scheduled shot {schedule_id}: {e}", exc_info=True
            )
...
async def apply_recommendations(
    ...
    try:
        ...
        if not isinstance(recs, list):
            raise HTTPException(
                status_code=400, detail="recommendations must be a JSON array"
            )
    except (json.JSONDecodeError, TypeError) as exc:
        raise HTTPException(
            status_code=400, detail=f"Invalid recommendations JSON: {exc}"
        )
    ...
    try:
        ...
        if matching_profile is None:
            raise HTTPException(
                status_code=404, detail=f"Profile '{name}' not found on machine"
            )
        ...
        for rec in recs:
            if not isinstance(rec, dict):
                skipped.append(
                    {"variable": "?", "reason": "invalid entry (not an object)"}
                )
                continue
            ...
            try:
                ...
                skipped.append(
                    {"variable": variable, "reason": "invalid recommended_value"}
                )
                continue
            ...
            if stage == "global":
                if variable == "temperature" and hasattr(full_profile, "temperature"):
                    ...
                    if val > 100:
                        skipped.append(
                            {"variable": variable, "reason": "exceeds 100 °C"}
                        )
                        continue
                    full_profile.temperature = val
                    applied.append({"variable": variable, "stage": stage, "value": val})
                    continue
                elif variable == "final_weight" and hasattr(
                    full_profile, "final_weight"
                ):
                    try:
                        val = float(recommended_value)
                    except (TypeError, ValueError):
            ...
            if hasattr(full_profile, "variables") and full_profile.variables:
                ...
                for var in full_profile.variables:
                    ...
                    if var_key == variable:
                        ...
                        if (
                            var_key.startswith("info_")
                            or getattr(var, "adjustable", None) is False
                        ):
                            skipped.append(
                                {
                                    "variable": variable,
                                    "reason": "info-only / not adjustable",
                                }
                            )
                            matched_var = True
                            break
                        try:
                            ...
                            break
                        applied.append(
                            {"variable": variable, "stage": stage, "value": var.value}
                        )
                        matched_var = True
                        break
                if matched_var:
            ...
            if (
                trigger_type
                and stage
                and hasattr(full_profile, "stages")
                and full_profile.stages
            ):
                matched_stage = False
                    ...
                    if stage_name.lower() == stage_lower:
                        ...
                        if not matched_stage:
                            skipped.append(
                                {
                                    "variable": variable,
                                    "reason": f"no {trigger_type} exit trigger in stage '{stage_name}'",
                                }
                            )
                            matched_stage = True
                        break
                    ...
                    if stage_name.lower() == stage_lower:
                        ...
                        if not matched_stage:
                            skipped.append(
                                {
                                    "variable": variable,
                                    "reason": f"no {limit_type} limit in stage '{stage_name}'",
                                }
                            )
                            matched_stage = True
                        break
            ...
            skipped.append(
                {"variable": variable, "reason": "variable not found in profile"}
            )
