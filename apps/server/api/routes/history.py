    ...
    try:
        ...
        for entry in entries:
            ...
            if "image_preview" in entry_copy:
                entry_copy["image_preview"] = (
                    None  # Remove for list view to save bandwidth
                )
            # Ensure profile_name is always a string (defense against corrupt data)
            if not entry_copy.get("profile_name"):
                entry_copy["profile_name"] = (
                    entry_copy.get("profile_json", {}).get("name", "Untitled Profile")
                    if isinstance(entry_copy.get("profile_json"), dict)
                    else "Untitled Profile"
                )
...
async def clear_history(request: Request):
    ...
    try:
        logger.warning("Clearing all history", extra={"request_id": request_id})
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "error": str(e),
                "message": "Failed to migrate history",
            },
        )
