...
async def _probe_watchtower_api(method: str = "get") -> dict:
    ...
    async with httpx.AsyncClient() as client:
        tasks = [
            asyncio.create_task(_probe_one(client, endpoint))
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                for release in releases:
                    ...
                    if not tag:
                        continue
                    is_pre = release.get("prerelease", False) or _is_prerelease_version(
                        tag
                    )
                    if is_pre:
...
async def get_tailscale_status(request: Request):
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"Tailscale status check failed: {e}", extra={"request_id": request_id}
            )
        ...
        if not status["installed"]:
            ...
            if os.path.exists(ts_socket):
                try:
                    ...
                    logger.debug(
                        f"Tailscale socket check failed: {e}",
                        extra={"request_id": request_id},
                    )
    ...
    try:
        ...
        if compose_changed:
            ...
            if ts_enabled and ts_flag not in current_compose:
                ...
                current_compose = current_compose.replace(f" {ts_flag}", "").replace(
                    ts_flag, ""
                )
        ...
        if env_updated:
            try:
                ...
                logger.info(
                    "Updated .env with Tailscale config",
                    extra={"request_id": request_id},
                )
        ...
        if compose_changed:
            try:
                ...
                logger.info(
                    "Scheduled container restart for Tailscale config change",
                    extra={"request_id": request_id},
                )
            except Exception as e:
                logger.warning(
                    f"Could not schedule restart: {e}", extra={"request_id": request_id}
                )
...
async def get_logs(
    ...
    try:
        ...
        return {
            ...
            "log_file": str(log_file),
            "filters": {"lines_requested": lines, "level": level, "log_type": log_type},
        }
...
async def get_version_info(request: Request):
    ...
    try:
        ...
        is_beta_version = any(
            suffix in version.lower() for suffix in ["-beta", "-alpha", "-rc"]
        )
...
async def get_changelog(request: Request):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                def strip_installation_section(body: str) -> str:
                    ...
                    pattern2 = r"\n### Installation.*$"
                    cleaned = re.sub(
                        pattern2, "", cleaned, flags=re.DOTALL | re.IGNORECASE
                    )
        ...
        if _changelog_cache:
            return _changelog_cache
        return {"releases": [], "error": str(e), "cached_at": None}
    ...
    try:
        ...
        if body.get("meticulousIp"):
            ...
            if "METICULOUS_IP=" in env_content:
                env_content = re.sub(
                    r"METICULOUS_IP=.*", f"METICULOUS_IP={new_ip}", env_content
                )
            else:
        ...
        if body.get("serverIp"):
            ...
            if "PI_IP=" in env_content:
                env_content = re.sub(r"PI_IP=.*", f"PI_IP={new_ip}", env_content)
            else:
                env_content += f"\nPI_IP={new_ip}"
            env_updated = True
        ...
        if body.get("meticulousIp"):
            ...
            try:
                ...
                logger.warning(
                    f"Failed to reset Meticulous API client: {e}",
                    extra={"request_id": request_id},
                )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info(
                        "Restarted MCP server with new METICULOUS_IP",
                        extra={"request_id": request_id, "new_ip": new_ip},
                    )
                else:
                    logger.warning(
                        f"Failed to restart MCP server: {result.stderr}",
                        extra={"request_id": request_id},
                    )
            except Exception as e:
                logger.warning(
                    f"Could not restart MCP server: {e}",
                    extra={"request_id": request_id},
                )
        ...
        if "mqttEnabled" in body:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to restart bridge: {e}", extra={"request_id": request_id}
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to reset MQTT subscriber: {e}",
                    extra={"request_id": request_id},
                )
...
"""
                ...
                logger.info(
                    "Created docker-compose.channel.yml for beta tag",
                    extra={"request_id": request_id},
                )
            else:
                ...
                if override_path.exists():
                    ...
                    compose_updated = True
                    logger.info(
                        "Removed docker-compose.channel.yml, reverting to latest",
                        extra={"request_id": request_id},
                    )
        except PermissionError:
            logger.warning(
        ...
        return {
            ...
            + (
                "Watchtower will pull the next beta update automatically."
                if enabled
                else "Watchtower will pull from the stable channel."
            )
            + (
                " Container restart may be required for changes to take effect."
                if compose_updated
                else ""
            ),
        }
...
async def send_feedback(request: Request):
    ...
    try:
        ...
        if not title or not description:
            raise HTTPException(
                status_code=400, detail={"error": "Title and description are required"}
            )
