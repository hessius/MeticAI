...
STYLE_MODIFIERS: Dict[str, Dict[str, List[str]]] = {
    ...
    "minimalist": {
        ...
        "artists": [
            "Malevich inspired",
            "Agnes Martin subtlety",
            "Donald Judd precision",
        ],
        "approaches": [
            "less is more",
...
class PromptBuilder:
    ...
    def _collect_influences(self) -> None:
        ...
        if not self.collected_influences:
            self.collected_influences.append(
                FLAVOR_INFLUENCES.get("caramel", TagInfluence())
            )
            self.collected_influences.append(
                TECHNIQUE_INFLUENCES.get("espresso", TagInfluence())
            )
    ...
    def build(self) -> str:
        ...
        prompt_parts.append(f'"{self.profile_name}" {profile_emphasis}')

        # 2. Art style with modifiers
        prompt_parts.append(f"{self.style} art style, {', '.join(style_modifiers)}")
...
def build_image_prompt_with_metadata(
    profile_name: str, style: str, tags: List[str]
) -> Dict:
    """
...
def build_taste_context(
    ...
    lines.append(
        "- Quadrant: Sour + Weak = severely under-extracted, needs major grind/temp adjustment"
    )
    lines.append(
        "- Quadrant: Sour + Strong = high-dose under-extraction, grind finer or increase temp"
    )
    lines.append(
        "- Quadrant: Bitter + Weak = channeling likely, improve puck prep and lower temp"
    )
    lines.append(
        "- Quadrant: Bitter + Strong = over-extracted and over-dosed, coarsen grind and reduce dose"
    )
    lines.append("")
    lines.append(
        "Based on this taste feedback, include a '## 6. Taste-Based Recommendations' section in your analysis that:"
    )
    ...
    *,
    ...
    if iterations:
        ...
        for it in iterations:
            ...
            balance_desc = _describe_axis_value(taste.get("x", 0), "Sour", "Bitter")
