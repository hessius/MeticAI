...
class TestProfileAPI:
    ...
    async def test_profile_schema_validation(self, wait_for_machine):
        ...
        if hasattr(profile, "__dict__"):
            ...
            dict(profile) if hasattr(profile, "keys") else {}
    ...
    async def test_last_shot_endpoint(self, meticulous_base_url):
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 404:
                ...
                response = await client.get(
                    f"{meticulous_base_url}/api/v1/history/list"
                )
                if response.status_code == 404:
...
class TestTelemetry:
    ...
    async def test_weight_data_available(self, integration_api):
        ...
        try:
            ...
            sensors = (
                ...
                else vars(getattr(data, "sensors", {}))
            )
    ...
    async def test_temperature_data_available(self, integration_api):
        ...
        try:
            ...
            assert "t" in sensors, (
                f"Temperature field 't' not in sensors: {sensors.keys()}"
            )
    ...
    async def test_pressure_data_available(self, integration_api):
        ...
        try:
            ...
            assert "p" in sensors, (
                f"Pressure field 'p' not in sensors: {sensors.keys()}"
            )
    ...
    async def test_telemetry_polling_latency(self, integration_api):
        ...
        try:
            ...
            assert len(timestamps) >= 2, (
                f"Only received {len(timestamps)} status events in 3s"
            )
    ...
    async def test_scale_weight_polling(self, integration_api):
        ...
        try:
            ...
            assert len(weights) >= 2, (
                f"Only received {len(weights)} weight readings in 2s"
            )
            ...
            print(
                f"Collected {len(weights)} weight readings, range: {min(weights):.1f}g - {max(weights):.1f}g"
            )
        finally:
            integration_api.sio.on("status", None)
    ...
    async def test_machine_state_complete(self, meticulous_base_url, integration_api):
        ...
        try:
            ...
            state_fields = (
                list(data.keys())
                if isinstance(data, dict)
                else [k for k in vars(data).keys() if not k.startswith("_")]
            )
            print(f"Socket.IO status fields: {state_fields}")
            print(
                f"Socket.IO status sample: {json.dumps(data if isinstance(data, dict) else vars(data), indent=2, default=str)[:500]}"
            )
        finally:
            integration_api.sio.on("status", None)
