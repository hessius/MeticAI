    ...
    async def save(self, scheduled_shots: dict) -> None:
        ...
        async with self._lock:
            try:
                ...
                active_shots = {
                    shot_id: shot
                    for shot_id, shot in scheduled_shots.items()
                ...
                logger.debug(
                    f"Persisted {len(active_shots)} scheduled shots to {self.persistence_file}"
                )
    ...
    async def load(self) -> dict:
        ...
        async with self._lock:
            try:
                ...
                logger.info(
                    f"Loaded {len(data)} scheduled shots from {self.persistence_file}"
                )
                return data
    ...
    async def save(self, schedules: dict) -> None:
        ...
        async with self._lock:
            try:
                ...
                enabled_count = sum(
                    1 for s in schedules.values() if s.get("enabled", True)
                )
                logger.debug(
                    f"Persisted {len(schedules)} recurring schedules ({enabled_count} enabled)"
                )
            except Exception as e:
...
async def restore_scheduled_shots():
    ...
    if _scheduled_shots:
        logger.info(
            f"Restored {len(_scheduled_shots)} scheduled shots from persistence"
        )
            ...
            if recurrence_type == "daily":
                ...
                if last_run:
                    try:
                        ...
                        logger.warning(
                            f"Invalid last_run format for schedule {schedule.get('id')}: {last_run}"
                        )
                        return candidate
                else:
