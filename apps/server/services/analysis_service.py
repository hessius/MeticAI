...
def _generate_execution_description(
    ...
    if abs(pressure_delta) > 0.5:
        if pressure_delta > 0:
            descriptions.append(
                f"Pressure rose from {start_pressure:.1f} to {end_pressure:.1f} bar"
            )
        else:
            descriptions.append(
                f"Pressure declined from {start_pressure:.1f} to {end_pressure:.1f} bar"
            )
    elif max_pressure > 0:
        descriptions.append(
            f"Pressure held around {(start_pressure + end_pressure) / 2:.1f} bar"
        )
    ...
    if abs(flow_delta) > 0.3:
        if flow_delta > 0:
            descriptions.append(
                f"Flow increased from {start_flow:.1f} to {end_flow:.1f} ml/s"
            )
        else:
            descriptions.append(
                f"Flow decreased from {start_flow:.1f} to {end_flow:.1f} ml/s"
            )
    elif max_flow > 0:
...
def _analyze_stage_execution(
    ...
    exit_triggers = _format_exit_triggers(
        profile_stage.get("exit_triggers", []), variables
    )
    ...
    if profile_stage.get("exit_triggers"):
        exit_result = _determine_exit_trigger_hit(
            shot_stage_data, profile_stage.get("exit_triggers", []), variables=variables
        )
    ...
    if result["exit_trigger_result"] and result["exit_trigger_result"]["triggered"]:
        ...
        result["exit_trigger_result"] and result["exit_trigger_result"]["not_triggered"]
    ):
        ...
        if dynamics_points and len(dynamics_points) >= 1:
            ...
            raw_target = (
                dynamics_points[-1][1]
                if len(dynamics_points[-1]) > 1
                else dynamics_points[-1][0]
            )
            ...
            if stage_type == "pressure":
                ...
                if (
                    target_value > 0 and max_pressure >= target_value * 0.95
                ):  # Within 5%
                    goal_reached = True
        ...
        if goal_reached:
            ...
            result["assessment"] = {
                "status": "failed",
                "message": f"Stage ended before exit triggers were satisfied. {goal_message}"
                if goal_message
                else "Stage ended before exit triggers were satisfied",
            }
    else:
        result["assessment"] = {
            "status": "executed",
...
def _interpolate_weight_to_time(
        ...
        if weight_actual >= target_weight:
            if i == 0:
                ...
                if weight_actual > weight_prev:
                    ...
                    weight_fraction = (target_weight - weight_prev) / (
                        weight_actual - weight_prev
                    )
...
def _generate_profile_target_curves(
    ...
    for stage in stages:
        ...
        # For weight-based dynamics, map weight values to time using actual shot data
            ...
            if (
                not stage_key_normalized
                or not stage_weight_to_time[stage_key_normalized]
            ):
            ...
            if len(dynamics_points) == 1:
                ...
                for dp in dynamics_points:
                    ...
                    actual_time = _interpolate_weight_to_time(
                        dp_weight, weight_time_pairs
                    )
...
def generate_estimated_target_curves(profile_data: dict) -> list[dict]:
    ...
    for idx, stage in enumerate(stages):
        ...
        dynamics_points = stage.get("dynamics_points", [])
        stage.get("dynamics_over", "time")

        if not dynamics_points:
            dynamics_obj = stage.get("dynamics", {})
            if isinstance(dynamics_obj, dict):
                dynamics_points = dynamics_obj.get("points", [])
                dynamics_obj.get("over", "time")
...
def _perform_local_shot_analysis(shot_data: dict, profile_data: dict) -> dict:
    ...
    profile_target_curves = _generate_profile_target_curves(
    ...
    for profile_stage in profile_stages:
        ...
        analysis = _analyze_stage_execution(
            profile_stage, shot_stage_data, total_time, profile_variables
        )
        ...
        is_preinfusion = any(kw in name_lower for kw in PREINFUSION_KEYWORDS) or any(
            kw in stage_key for kw in ["preinfusion", "bloom", "soak", "fill"]
        )
    ...
    preinfusion_proportion = (
        (preinfusion_time / total_time * 100) if total_time > 0 else 0
    )
    ...
    preinfusion_weight_percent = (
        (preinfusion_weight / final_weight * 100) if final_weight > 0 else 0
    )
    preinfusion_issues = []
    ...
    if preinfusion_weight_percent > 10:
        preinfusion_issues.append(
            {
                ...
                "severity": "warning"
                if preinfusion_weight_percent <= 15
                else "concern",
        ...
        max_preinfusion_flow = max(
            (s["max_flow"] for s in preinfusion_stages), default=0
        )
        avg_preinfusion_flow = (
            sum(s["avg_flow"] for s in preinfusion_stages) / len(preinfusion_stages)
            if preinfusion_stages
            else 0
        )
        ...
        if max_preinfusion_flow > 2.0 or avg_preinfusion_flow > 1.0:
            ...
            preinfusion_recommendations.append(
                "Consider using a finer grind to slow early flow"
            )
        ...
        for pi_stage in preinfusion_stages:
            for trigger in pi_stage.get("exit_triggers", []):
                trigger_type = (
                    trigger.get("type", "") if isinstance(trigger, dict) else ""
                )
        ...
        if not has_weight_exit and not has_flow_exit:
            preinfusion_recommendations.append(
                "Consider adding a weight or flow exit trigger to pre-infusion stages to prevent excessive early volume"
            )
...
def _prepare_shot_summary_for_llm(
    shot_data: dict, profile_data: dict, local_analysis: dict
) -> dict:
    ...
    for stage in local_analysis.get("stage_analyses", []):
        ...
        if exec_data:
            ...
            pct_of_shot = round(
                (duration / total_time * 100) if total_time > 0 else 0, 1
            )
            ...
            stage_summaries.append(
                {
                    ...
                    "cumulative_weight_at_end": exec_data.get(
                        "end_weight"
                    ),  # Added: cumulative weight when stage ended
            ...
            stage_summaries.append(
                {"name": stage.get("stage_name"), "status": "NOT REACHED"}
            )
            ...
            graph_summary.append(
                {
                    ...
                    "flow": round(
                        shot.get("flow", 0) or shot.get("gravimetric_flow", 0), 1
                    ),
...
def _build_static_profile_description(profile_json: dict) -> str:
    ...
    if existing_description:
            ...
            if isinstance(points, list) and len(points) >= 2:
                try:
                    ...
                    if pressures and all(
                        abs(p - pressures[0]) < 0.3 for p in pressures
                    ):
        ...
        if stages:
            ...
            if shot_traits:
                trait_str = ", ".join(
                    dict.fromkeys(shot_traits)
                )  # dedupe, preserve order
                description_parts.append(
                    f"A {stage_count_text} extraction featuring {trait_str}"
                )
            else:
    ...
    yield_text = (
        f"{final_weight}g" if final_weight is not None else "Use profile default"
    )
