...
import { POUR_OVER_RECIPES } from '@metic/core/data/recipes'
import {
...
/**
 * Peak of a pressure/flow stage's resolved dynamics setpoints (#423).
 *
 * Feeds effectiveControlMode() so an aggressive flow stage paired with a
 * pressure limit can be recognised as effectively pressure-controlled.
 * Mirror of the server _max_dynamics_target — keep the two in sync.
 */
...
export function installDirectModeInterceptor(): void {
  ...
  window.fetch = function directModeFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    if (url.match(/\/api\/recipes$/)) {
      return Promise.resolve(jsonResponse(POUR_OVER_RECIPES))
    }
