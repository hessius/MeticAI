...
# ─── Generate dSYMs for prebuilt dynamic frameworks (App Store upload) ──
# Some SPM binaryTarget xcframeworks ship a stripped dynamic framework with no
# accompanying dSYM (e.g. CLiteRTLM from LiteRT-LM, pulled in by @capgo/
...
PHASE_ID = 'D50EDE0000000000DE5C0001'

# Shell script run during build/archive. Escaped for inclusion as a pbxproj
# double-quoted string (literal \n between lines, escaped quotes/backslashes).
script_lines = [
    '# Auto-generated by patch-capacitor-spm.sh: synthesize dSYMs for prebuilt',
    '# dynamic frameworks (e.g. CLiteRTLM) so App Store upload validation passes.',
    'set -u',
...
script_str = '\\n'.join(esc(l) for l in script_lines) + '\\n'

phase_obj = (
    '/* Begin PBXShellScriptBuildPhase section */\n'
    '\t\t' + PHASE_ID + ' /* Generate framework dSYMs */ = {\n'
    ...
    '\t\t\tinputFileListPaths = (\n\t\t\t);\n'
    '\t\t\tinputPaths = (\n\t\t\t);\n'
    '\t\t\tname = "Generate framework dSYMs";\n'
    '\t\t\toutputFileListPaths = (\n\t\t\t);\n'
    '\t\t\toutputPaths = (\n\t\t\t);\n'
    '\t\t\trunOnlyForDeploymentPostprocessing = 0;\n'
    '\t\t\tshellPath = /bin/sh;\n'
    '\t\t\tshellScript = "' + script_str + '";\n'
    '\t\t};\n'
...
# 1. Insert the shell-script phase section (create it fresh; the project has
#    none by default) just before the PBXNativeTarget section.
if 'PBXShellScriptBuildPhase section' not in content:
    anchor = '/* Begin PBXNativeTarget section */'
    if anchor in content and PHASE_ID not in content:
        content = content.replace(anchor, phase_obj + '\n' + anchor, 1)
        changed = True

# 2. Reference the phase in the native target's buildPhases list (append last).
def add_phase(match):
    block = match.group(0)
    if PHASE_ID in block:
        return block
...
new_content = re.sub(r'buildPhases = \(\n.*?\n\t\t\t\);', add_phase, content,
...
if changed:
    with open(path, 'w') as f:
        f.write(content)
    print("  Added 'Generate framework dSYMs' build phase to App target")
else:
