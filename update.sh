...
# ──────────────────────────────────────────────────────────────────────────────
is_v1_migration() {
    # v1 artifacts that only exist in old installations
    [[ -f "$SCRIPT_DIR/rebuild-watcher.sh" ]] && return 0
    [[ -f "$SCRIPT_DIR/local-install.sh" ]] && return 0
    # Old multi-container compose (coffee-relay, meticai-web services)
    [[ -f "$SCRIPT_DIR/docker-compose.yml" ]] \
        && grep -q 'coffee-relay\|meticai-web' "$SCRIPT_DIR/docker-compose.yml" 2>/dev/null \
        && return 0
    return 1
}
